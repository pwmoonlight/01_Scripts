...
dir.create("10_Background_Data", showWarnings=F)
dir.create("10_Background_Data/Biased", showWarnings=F)
dir.create("10_Background_Data/Non_Biased", showWarnings=F)

for(x in length(species):1){
  writeLines(paste("\nFinding Background data for ", species[[x]], ":", sep=""))
  
  circle <- read.csv(paste("08_Species_To_Model_Non_Scale_Corrected_Distribution_Data/", species[[x]], ".csv", sep=""))[,-1]
  ...
  writePolyShape(as(circle, "SpatialPolygonsDataFrame"), paste("05_Species_Circles/", species[[x]], ".shp", sep=""))
  ...
  write.csv(background_biased, file=paste("10_Background_Data/Biased/", species[[x]], ".csv", sep=""))
  write.csv(background_not_biased, file=paste("10_Background_Data/Non_Biased/", species[[x]], ".csv", sep=""))
}


rm(bg, background, background_biased, background_not_biased, circle, x)
