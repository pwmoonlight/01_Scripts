...
distribution_data <- distribution_data[,c(35,34,23,21,12,13)]

# Remove specimens not determined to species
for(x in length(distribution_data[,6]):1){
  if(is.na(distribution_data[x,6])){distribution_data <- distribution_data[-x,]}
  if(distribution_data[x,6] ==""){distribution_data <- distribution_data[-x,]}
  if(distribution_data[x,6] =="sp"){distribution_data <- distribution_data[-x,]}
  if(distribution_data[x,6] =="sp."){distribution_data <- distribution_data[-x,]}
  
  if(grepl("cf.", distribution_data[x,6])){distribution_data <- distribution_data[-x,]}
  if(grepl("aff.", distribution_data[x,6])){distribution_data <- distribution_data[-x,]}
  if(grepl("subsp.", distribution_data[x,6])){distribution_data <- distribution_data[-x,]}
  if(grepl("var.", distribution_data[x,6])){distribution_data <- distribution_data[-x,]}
  if(grepl(" ", distribution_data[x,6])){distribution_data <- distribution_data[-x,]}
}

distribution_data[,7] <- paste(distribution_data[,5], distribution_data[,6])
colnames(distribution_data) <- c("lat","long","collector", "collectornumber", "genus", "species", "scientific_name")
