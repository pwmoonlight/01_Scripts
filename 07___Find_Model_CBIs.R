...
for(x in 1:length(species)){
  AUCs <- read.csv(paste("11_models/Bias_Spatial_Filtering/", species[[x]], "/maxentResults.csv", sep=""))
...
for(x in 1:length(species)){
  ...
  for(y in 0:4){
    ...
    model <- raster(paste("11_models/Bias_Spatial_Filtering/", species[[x]], "_", y+1, ".tif", sep=""))
    
        
    partitions <- read.csv(paste("11_models/Bias_Spatial_Filtering/", species[[x]], "/species_", y, "_samplePredictions.csv", sep=""))
  ...
  write.csv(CBI_results, file="CBI_results.csv")
}
