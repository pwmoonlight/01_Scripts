...
distribution_data <- list.files('02_Input_Distribution_Data/REFLORA', pattern="[.]csv", full.names=T)
distribution_data <- read.csv(distribution_data, header=T)

distribution_data <- distribution_data[,c(32,31,13,12,39,40,41,27,28,24)]

# Remove specimens with no latitude or longitude
for(y in length(distribution_data[,7]):1){if(is.na(distribution_data[y,1])){distribution_data <- distribution_data[-y,]}}
for(y in length(distribution_data[,7]):1){if(distribution_data[y,1] ==""){distribution_data <- distribution_data[-y,]}}
for(y in length(distribution_data[,7]):1){if(is.na(distribution_data[y,2])){distribution_data <- distribution_data[-y,]}}
for(y in length(distribution_data[,7]):1){if(distribution_data[y,2] ==""){distribution_data <- distribution_data[-y,]}}


# Remove specimens not determined to species
for(y in length(distribution_data[,7]):1){if(is.na(distribution_data[y,6])){distribution_data <- distribution_data[-y,]}}
for(y in length(distribution_data[,7]):1){if(distribution_data[y,6] == ""){distribution_data <- distribution_data[-y,]}}
for(y in length(distribution_data[,7]):1){if(distribution_data[y,6] == "sp."){distribution_data <- distribution_data[-y,]}}
...
#Save the data
dir.create("03_Modelling", showWarnings = F)
...
dir.create("03_Modelling/04_Species_To_Model_Distribution_Data/Uncleaned/REFLORA", showWarnings = F)
...
lapply(species[not_previously_modelled_species], function(y){dir.create(paste("03_Modelling/04_Species_To_Model_Distribution_Data/Uncleaned/REFLORA/", y, sep=""), showWarnings = F)})
lapply((1:length(species)), function(y){
  if(not_previously_modelled_species[[y]]){
    write.csv(species_distribution_data[[y]], paste("03_Modelling/04_Species_To_Model_Distribution_Data/Uncleaned/REFLORA/", species[y], "/data.csv", sep=""))
  }})

writeLines("\n\n##############################################################")
writeLines("### The species previously modelled with REFLORA data are:\n")
...
for(y in 1:length(species_to_crossref)){
  ...
  if(!identical(old_data,new_data)){
    dir.create(paste("03_Modelling/04_Species_To_Model_Distribution_Data/Uncleaned/REFLORA/", species_to_crossref[[y]], sep=""), showWarnings = F)
    write.csv(new_data,paste("03_Modelling/04_Species_To_Model_Distribution_Data/Uncleaned/REFLORA/", species_to_crossref[[y]], "/data.csv", sep=""))
