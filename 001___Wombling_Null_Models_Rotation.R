...
SDM.b <- brick("SDMb_2016June21.grd")
SDM.b
nlayers(SDM.b)
#names(SDM.b)
...
species.in.analysis <- read.table("SpeciesInAnalysis_2016June21.txt", header=T, sep=",")
...
MCLUST.model.names <- paste("MCLUST", species.genus.family, sep="_")
...
for(null.iterations in 1:iterations.to.perform)
{
	...
	for(h in 1: length(MCLUST.model.names))
	{
		...
		if(AOO.fraction.to.add < 1) writeRaster(occurrences.r, filename=paste(species.genus.family[h], "_NULL.grd", sep=""))
		...
		writeRaster(occurrences.r, filename=paste(species.genus.family[h], "_NULL.grd", sep=""))
		rm(occurrences.r)
	}
	...
	setwd(path.for.NullDistributions)
	SDM.raster.names <- vector(mode = "list", length = length(seq(1, length(dir()), 2)))
	j <- 1
	for(i in seq(1, length(dir()), 2))
	{
		SDM.raster.names[[j]]  <- dir()[i]
		j <- j + 1
	}
	...
	writeRaster(Null_SDM.b,	filename=paste("Null_", random.number, "_", format(Sys.time(), "%d%b%Y_%H%M%S"), "_SDMb.grd", sep=""), bandorder='BIL')
	#remove null brick
	rm(Null_SDM.b)
	...
	write.table(final.j, file = paste("Null_", random.number, "_", format(Sys.time(), "%d%b%Y_%H%M%S"), "_final_j", sep=""), quote = TRUE, sep = ",")
