...
dir.create("03_Modelling/98_PCA", showWarnings=F)

writeLines(paste("...Reading in distribution data"))

species_distribution_data <- list.files("03_Modelling/09_Species_To_Model_Scale_Corrected_Distribution_Data", pattern="*.csv", full.names = T)
...
write.csv(values, "03_Modelling/98_PCA/Background_variables_for_collectionpoints.csv", quote=F, row.names=F)

writeLines(paste("...Standardising background data values"))
values_std <- scale(values)
write.csv(values_std, "03_Modelling/98_PCA/Background_variables_for_collectionpoints_standardised.csv", quote=F, row.names=F)

writeLines(paste("...Finding Correlations"))
correlation <- cor(values)
write.csv(correlation, "03_Modelling/98_PCA/correlations.csv", quote=F)
...
write.csv(pca_loadings, "03_Modelling/98_PCA/pca_loadings.csv")
